...
local function createTabButton(name)
	local b = Instance.new("TextButton")
	b.Size = UDim2.new(1/6,0,1,0) -- automatically fits 6 tabs
	b.Text = name
	b.BackgroundColor3 = buttonColor
	b.TextColor3 = Color3.new(1,1,1)
	b.Parent = tabBar
...
local viewTabButton = createTabButton("View")
local serverTabButton = createTabButton("Server")
local funTabButton = createTabButton("Fun")
...
-- FUN TAB
local funTab = Instance.new("ScrollingFrame")
funTab.Size = UDim2.new(1,0,1,0)
funTab.Visible = false
funTab.Active = true
funTab.CanvasSize = UDim2.new(0,0,0,0)
funTab.AutomaticCanvasSize = Enum.AutomaticSize.Y
funTab.ScrollBarThickness = 6
funTab.BackgroundTransparency = 1
funTab.Parent = container
funTab.ScrollingEnabled = true
funTab.ScrollBarImageTransparency = 0
...
followInput.Size = UDim2.new(0,220,0,35)
followInput.Position = UDim2.new(0.5, 0, 0, funY)
followInput.AnchorPoint = Vector2.new(0.5, 0)
followInput.PlaceholderText = "Enter victim username"
followInput.Text = ""
followInput.TextScaled = true
followInput.BackgroundColor3 = buttonColor
followInput.TextColor3 = Color3.new(1,1,1)
followInput.Parent = funTab
outlineText(followInput)


funY = funY + 45

local followButton = makeButton("Follow Player", funCol1, funY, funTab)
local bangButton = makeButton("Bang Player", funCol2, funY, funTab)
funY = funY + 50

local banging = false
local bangTarget = nil
local bangDistance = 3
local bangAmplitude = 2
local bangSpeed = 6
local bangTime = 0

-- SPEED CONTROL (but for bang)

local speedLabel = Instance.new("TextLabel")
speedLabel.Size = UDim2.new(0,180,0,25)
speedLabel.Position = UDim2.new(0,funCol2,0,funY - 17)
speedLabel.Text = "Bang Speed: "..bangSpeed
speedLabel.BackgroundTransparency = 1
speedLabel.TextColor3 = Color3.new(1,1,1)
speedLabel.Parent = funTab
outlineText(speedLabel)

local speedMinus = makeButton("-", funCol2, funY + 5, funTab)
speedMinus.Size = UDim2.new(0,80,0,30)

local speedPlus = makeButton("+", funCol2 + 90, funY + 5, funTab)
...
speedMinus.MouseButton1Click:Connect(function()
	bangSpeed = math.max(1, bangSpeed - 2)
	speedLabel.Text = "Bang Speed: "..bangSpeed
end)
...
local spinButton = makeButton("Spin", funCol1, funY - 60, funTab)
funY = funY + 45
...
RunService.RenderStepped:Connect(function()
	...
	if following and followTarget and followTarget.Character and player.Character then
		...
		if hrp and targetHRP then
			...
			local behindPosition = targetHRP.Position - (targetHRP.CFrame.LookVector * followDistance)

			local goalCFrame = CFrame.new(behindPosition, targetHRP.Position)
...
-- RunService loop for banging
RunService.RenderStepped:Connect(function(dt)
	if banging and bangTarget and bangTarget.Character and player.Character then
		local hrp = player.Character:FindFirstChild("HumanoidRootPart")
		local targetHRP = bangTarget.Character:FindFirstChild("HumanoidRootPart")

		if hrp and targetHRP then
			bangTime += dt

			local basePos = targetHRP.Position - targetHRP.CFrame.LookVector * bangDistance
			local offset = math.sin(bangTime * bangSpeed) * bangAmplitude

			local bangPos = basePos + targetHRP.CFrame.LookVector * offset
			local cf = CFrame.new(bangPos, targetHRP.Position)

			hrp.CFrame = hrp.CFrame:Lerp(cf, 0.35)
		end
	end
end)
